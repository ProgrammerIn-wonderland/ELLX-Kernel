...
/**
 * struct geni_spi_desc - per-compatible resource ops and behavioral flags
 * @resources_init: acquire clocks/interconnect or attach power domains
 * @set_rate: program the serial clock to the requested frequency
 * @power_on: runtime-resume hook (optional)
 * @power_off: runtime-suspend hook (optional)
...
{
	return mas->dev_data && mas->dev_data->qspi_mode;
}
...
static const struct geni_spi_desc geni_spi_qspi = {
	.resources_init = geni_se_resources_init,
	.set_rate = geni_spi_set_clock_and_bw,
	.power_on = geni_se_resources_activate,
	.power_off = geni_se_resources_deactivate,
...
static const struct of_device_id spi_geni_dt_match[] = {
	...
	{ .compatible = "qcom,geni-spi-qspi", .data = &geni_spi_qspi },
